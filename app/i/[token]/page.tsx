...
function MessageScreen({ title, description, action, headerTitle = '초대 링크' }: { title: string; description: string; action?: React.ReactNode; headerTitle?: string }) {
  return (
    <><TopBar title={headerTitle} backHref="/friends" />
    <main className="mx-auto flex w-full max-w-[430px] flex-1 flex-col items-center justify-center px-8 pb-16 text-center">
...
export default async function InvitePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!preview) return <MessageScreen title="더 이상 쓸 수 없는 링크예요" description="보낸 분께 새 링크를 요청해주세요." action={<LinkButton href="/friends/invite" size="md" className="w-auto px-[18px]">링크 생성하기</LinkButton>} />
  ...
  return (
    <><TopBar title="초대 완료" backHref="/friends" />
    <main className="mx-auto flex w-full max-w-[430px] flex-1 flex-col items-center justify-center px-8 pb-16 text-center">
      ...
      <div className="w-full pt-8"><LinkButton href={`/friends/${result.data.friendUserId}`} size="lg">{preview.displayName}님 취향 보기</LinkButton><LinkButton href="/" variant="tertiary" className="mt-2">홈으로</LinkButton></div>
    </main><BottomNav active="friends" /></>
  )
}
