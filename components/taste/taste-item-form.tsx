      ...
      <BottomSheet isOpen={isOpen} onClose={onClose} labelledBy={titleId}>
        ...
        <div className="flex flex-col gap-4">
          ...
          <fieldset className="flex flex-col gap-1.5">
            ...
            <div className="flex items-center gap-6">
              ...
              <RadioOption
                ...
                onChange={() =>
                  setDraft((prev) => ({ ...prev, kind: "UNWANTED" }))
                }
              />
            </div>
          </fieldset>
          <TextField
            ...
            value={draft.detail}
            onChange={(event) =>
              setDraft((prev) => ({ ...prev, detail: event.target.value }))
            }
          />
          <TextField
            id={memoId}
...
export function WishlistItemSheet({
  ...
  return (
    <>
      <BottomSheet isOpen={isOpen} onClose={onClose} labelledBy={titleId}>
        <h2 id={titleId} className="pb-4 text-lg font-bold text-neutral-900">
          {isEditing ? "원하는 것 편집" : "원하는 것을 적어주세요"}
        </h2>
        <div className="flex flex-col gap-4">
          <CategoryPicker
            categories={categories}
...
export function AddTasteItemButton({
  kind,
  categories,
}: AddTasteItemButtonProps) {
...
/** 목록 행 — 탭하면 편집 시트를 연다 */
export function TasteItemRowButton({
  item,
  categories,
}: TasteItemRowButtonProps) {
  const [isOpen, setIsOpen] = useState(false);
  const label =
    item.kind === "WANT"
  ...
  const SheetComponent =
    item.kind === "WANT" ? WishlistItemSheet : TasteItemSheet;
  return (
    <>
      <button
        type="button"
      ...
      >
        <span
          aria-hidden
          className="size-1.5 shrink-0 rounded-full bg-rose-300"
        />
