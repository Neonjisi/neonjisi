...
- **E2E 상태 (2026-09-06)**: 마지막으로 끝까지 돈 전체 실행은 **74 통과 · 4 실패 · skip 0**
  이었다. 그 뒤 `gift-request` 2건을 고쳤고(아래), 이후 전체 실행은 중간에 끊겨 확정된
  최종 수치가 없다. 파일 묶음으로 나눠 돌린 결과로는 `gift-*`·`home-hub`·`funding-*` 이
  전부 초록이다. **T042 수동 검증을 막는 것은 없다.**

  - `gift-request:146`·`:174` — **해결.** 화면 동작이 아니라 두 겹의 테스트 문제였다.
    ① 예산: 실패 건을 DB 에서 열어보니 `resolution=COUNTERED` · `counterAmount=12,000` ·
    `status=PAID` · 결제 `PAID` · 배송지 스냅샷까지 정상이었고 `/result` 로의 RSC 이동만
    기본 5초를 넘겼다 → 20초. ② 그러자 처음 실행된 다음 줄에서 Next 라우트 안내자가
    같은 문구를 복제해 `getByText` 가 2개에 걸렸다 → `getByRole('heading')`.
  - `invite-control:302`(빈 알림 목록 기대) — 매번 재현된다. e2e2 에 안 읽은 알림 126건.
  - `invite-control:144`·`:282` — **불안정하다.** 실행에 따라 통과·실패가 갈린다.
    누적된 초대 링크 `usedCount`(A 기준 13)와 알림 수에 따라 달라지는 것으로 보이나
    확정하지 못했다.
  - `funding-create` — 실패가 파일 안에서 옮겨다닌다(`:68` 한 번, 다음 실행엔 `:48`).
    `:48` 은 `deleteAllPaymentMethods` 가 10회 안에 못 끝내서 터졌다.

- **밑바닥 원인은 공유 계정 오염이다 (팀 논의 필요).** 스펙들이 자기 뒤를 치우지 않아
  실행할수록 계정이 나빠진다. 2026-09-06 기준 A 계정: 결제수단 **ACTIVE 1 · DELETED 157**,
  진행 중 선물 요청 **6건**, 안 읽은 알림 126건, 활성 초대 링크 `usedCount` 13.
  위 실패들(친구 수 · 알림 수 · 링크 카운트 · 결제수단 정리)이 전부 여기서 나온다.
  DB 를 한 번 청소해도 다음 실행이 다시 쌓으므로 임시 조치다. 근본 해법은 T029 에서
  `resetAccount` 를 `taste-db.ts` 로 옮긴 것과 같은 방향 — 계정 초기화를 DB 픽스처로
  만들어 스펙들이 쓰게 하는 것이다. **태스크 번호를 붙여 별도로 다뤄야 한다.**
